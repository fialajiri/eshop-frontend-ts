...
describe("user actions creators", () => {
  it("load current user success", async () => {
    const store = setup();
    const {store : mystore, reduxThunkTester: {getActionHistoryAsync, getActionHistoryStringifyAsync}} = set();
    ...
    expect(store.getActions()).toEqual([
      ...
      { type: ProductActionTypes.PRODUCT_LIST_SUCCESS, payload: mockProducts },
    ]);
  });
