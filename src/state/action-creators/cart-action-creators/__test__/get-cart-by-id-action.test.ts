...
it("load cart success", async () => {
    ...
    await store.dispatch(getCartById(mockCart.id))
    ...
    // @ts-ignore
    await reduxTestStore.dispatch(getCartById(mockCart.id));    
  ...
  it("load cart failed", async () => {
    ...
    server.resetHandlers(
      rest.get(`${process.env.BACKEND_URL}/api/cart/${mockCart.id}`, (req, res, ctx) =>
        res(ctx.status(500))
      )
    );
